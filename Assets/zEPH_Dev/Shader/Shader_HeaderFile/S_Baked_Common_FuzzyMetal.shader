Shader "Custom/Baked Common FuzzyMetal"
{
    // ---------------------------------------------------------------------------
    // 属性定义
    // ---------------------------------------------------------------------------
    Properties
    {
        ...
        [Space(5)]
        [KeywordEnum(B, A)] _MetallicSource("金属度来源通道 (ORM)", Float) = 0
        ...
        [Header(Oren Nayar)]
        [Space(5)]
        _OrenNayarRoughness("Oren Nayar 粗糙度", Range(0.0, 1.0)) = 0.5
        _RimIntensity("边缘光强度", Range(0.0, 2.0)) = 0.5
        _RimRange("边缘光范围", Range(0.0, 1.0)) = 1.0
        _RimExponent("边缘光指数", Range(0.0, 10.0)) = 5.0
        ...
        [Space(5)]
        [Toggle(_USE_BAKED_OREN_NAYAR)] _UseBakedOrenNayar("启用烘焙 Oren Nayar 补偿", Float) = 0
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Baked_Common_FuzzyMetal_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _METALLICSOURCE_A  // 金属度来源是否为 A 通道
            #pragma shader_feature_local _USE_BAKED_OREN_NAYAR // 启用烘焙 Oren Nayar 补偿
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Baked_Common_FuzzyMetal_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _ALPHATEST_ON
            #pragma shader_feature_local _ALPHASOURCE_A
            #pragma shader_feature_local _METALLICSOURCE_A
