Shader "Custom/Common Crystal"
{
    ...
    {
        [Header(Impurity)]
        [NoScaleOffset] _ImpurityMap("杂质贴图 (Noise)", 2D) = "white" {}
        _ImpurityScale("杂质缩放 (Scale)", Vector) = (1, 1, 0, 0)
        _ImpurityOffset("杂质偏移 (Offset)", Vector) = (0, 0, 0, 0)
        _InnerImpurityOffset("内部杂质偏移 (Inner Offset)", Vector) = (0.10667, 0.544, 0, 0)
        _ImpurityDepthInterval("杂质深度间隔距离 (Depth Interval)", Float) = 1.576309
        _ImpurityDepthOffset("杂质深度偏移 (Depth Offset)", Float) = 1.709498
        _CenterImpurityRange("中心杂质范围 (Center Range)", Float) = 1.5
        _CenterImpurityIntensity("中心杂质强度 (Center Intensity)", Float) = 1.0
        _ImpurityColorDark("杂质颜色 (暗部)", Color) = (0, 0, 0, 1)
        _ImpurityColorBright("杂质颜色 (亮部)", Color) = (0.0852, 0.0397, 0.155, 1)
        _EmissionScale("自发光强度", Float) = 1.0
        
        [Header(Bump Offset Impurity)]
        [Space(5)]
        _HeightMapScale("高度贴图缩放", Float) = 1.0
        _HeightMapOffset("高度贴图偏移", Vector) = (0.637334, 0.199972, 0, 0)
        _HeightRatio("高度比率 (Bump Offset Ratio)", Float) = 0.138667
        ...
        [Space(20)]
        [Toggle(_DEBUG_MODE_ON)] _DebugMode("调试模式 (Debug Mode)", Float) = 0
    }
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Common_Crystal_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _DEBUG_MODE_ON     // 调试模式
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Common_Crystal_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            // half4 LitPassFragment(Varyings IN, float facing : VFACE) : SV_Target
            {
                ...
                // Calculate Tangent Space View Direction (Always needed for Impurity)
                // Fix: Use standard ViewDir (normalized), do NOT scale by -0.5 here.
                // We will handle the scale inside the function for specific effects.
                half3 viewDirWS = GetWorldSpaceNormalizeViewDir(IN.positionWS);
                half3 normalWS = NormalizeNormalPerPixel(IN.normalWS);
                half3 tangentWS = NormalizeNormalPerPixel(IN.tangentWS.xyz);
                half3 bitangentWS = cross(normalWS, tangentWS) * IN.tangentWS.w;
                
                half3 viewDirTS;
                viewDirTS.x = dot(viewDirWS, tangentWS);
                viewDirTS.y = dot(viewDirWS, bitangentWS);
                viewDirTS.z = dot(viewDirWS, normalWS);

                // 初始化表面数据 (读取贴图、计算属性) - 定义在 zEPH_Optimized_Surface.hlsl
                InitializeSurfaceData(IN.uv, viewDirTS, surfaceData);
        ...
        Pass
        {
            ...
            half4 FragmentMeta(Varyings IN) : SV_Target
            {
                ...
                // Meta Pass doesn't have view direction, assume looking straight at surface
