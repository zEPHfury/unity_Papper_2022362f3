Shader "Custom/Baked Common Silk"
{
    // ---------------------------------------------------------------------------
    // 属性定义
    // ---------------------------------------------------------------------------
    Properties
    {
        ...
        [Space(5)]
        [Toggle(_USE_BAKED_SPECULAR)] _UseBakedSpecular("启用烘焙高光补偿 (Fake Specular)", Float) = 0
        _BakedLightDir("烘焙光源方向 (World Space)", Vector) = (1.0, 1, -1.0, 0)
        [HDR] _BakedLightColor("烘焙光源颜色", Color) = (1, 1, 1, 1)
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Baked_Common_Silk_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _USE_BAKED_SPECULAR // 启用烘焙高光补偿
            ...
            #pragma multi_compile _ _MIXED_LIGHTING_SUBTRACTIVE // 混合光照减去模式 (修正 MixRealtimeAndBakedGI 逻辑)
            ...
            struct Varyings
            {
                ...
                float3 normalWS     : TEXCOORD1;   // 世界空间法线 (High Precision)
                float4 tangentWS    : TEXCOORD2;   // 世界空间切线 (High Precision)
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Baked_Common_Silk_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
