Shader "Custom/Common ICE"
{
    ...
    Properties
    {
        ...
        _BaseColorBrightness("基础颜色亮度", Range(0.0, 10.0)) = 1.0
        ...
        _SmoothnessMin("光滑度最小值", Range(0.0, 1.0)) = 0.9
        _SmoothnessMax("光滑度最大值", Range(0.0, 1.0)) = 1.0
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Common_Ice_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Common_Ice_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            // half4 LitPassFragment(Varyings IN, float facing : VFACE) : SV_Target
            {
                ...
                InitializeSurfaceData(IN.uv, viewDirTS, viewDirWS, surfaceData);
        ...
        Pass
        {
            ...
            {
                SurfaceData surfaceData;
                // Meta Pass 不需要视差偏移，传默认值
                InitializeSurfaceData(IN.uv, half3(0,0,1), half3(0,0,1), surfaceData);
