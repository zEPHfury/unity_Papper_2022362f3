Shader "Custom/Common WaterTranslucent"
{
    // ---------------------------------------------------------------------------
    // 属性定义
    // ---------------------------------------------------------------------------
    Properties
    {
        ...
        _DepthMaxDistance ("颜色渐变最大距离", Range(0.01, 10)) = 0.2
        _DepthExp ("颜色渐变指数", Range(0.01, 10)) = 1.0
        _OpacityDepthDistance ("透明过渡最大距离", Range(0.01, 1)) = 0.2
        _OpacityExp ("透明过渡指数", Range(0.01, 10)) = 1.0
        [Toggle(_FIXED_DEPTH)] _FixedDepth ("固定水深度 (不受视角影响)", Float) = 1

        [Space(15)]
        [Header(Foam Settings)]
        [Toggle(_FOAM)] _UseFoam("启用泡沫", Float) = 0
        _FoamNoise ("泡沫噪声", 2D) = "white" {}
        _FoamNoiseScale ("泡沫缩放", Float) = 2.0
        _FoamColor ("泡沫颜色", Color) = (1, 1, 1, 1)
        _FoamNoiseSpeed ("泡沫流速", Vector) = (0.005, 0.005, 0, 0)
        _FoamDistance ("泡沫范围", Range(0, 2)) = 0.1
        _FoamNoiseMul ("泡沫强度倍数", Float) = 1.5
        _FoamNoisePow ("泡沫噪声指数", Range(0.01, 5)) = 1.0
        _FoamNoiseCutoff ("泡沫阈值", Range(0, 1)) = 0.5
        _FoamSmoothness ("泡沫边缘平滑度", Range(0.0, 2)) = 1
        _FoamShoreSmoothness ("泡沫近岸平滑 (交界处)", Range(0.0, 0.1)) = 0.001
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Common_WaterFormTranslucent_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _FIXED_DEPTH       // 是否使用固定深度计算
            #pragma shader_feature_local _FOAM              // 是否启用泡沫
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Common_WaterFormTranslucent_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
