...
#if defined(_FRESNEL_RIM)
    half NdotV = saturate(dot(inputData.normalWS, inputData.viewDirectionWS));
    half fresnelTerm = pow(1.0 - NdotV, _RimPower);
    half rimStrength = max(0.0h, lerp(_RimCenterBrightness, _RimEdgeBrightness, fresnelTerm));
    surfaceData.emission += _RimColor.rgb * surfaceData.albedo * rimStrength;
#endif
