Shader "zEPH/Dev_Subsuface_TerminatorFix"
{
    // ---------------------------------------------------------------------------
    // 属性定义
    // ---------------------------------------------------------------------------
    Properties
    {
        ...
        [HideInInspector] _EmissionMap_ST("Emission Map ST", Vector) = (1,1,0,0)
        ...
        [KeywordEnum(B, A)] _AlphaSource("Alpha 来源通道 (ORM)", Float) = 0 // 选择 Alpha 数据来源：ORM 的 B 通道还是 A 通道
        _Cutoff("裁剪阈值", Range(0.0, 1.0)) = 0.3

        [Space(15)]
        [Header(Subsurface Scattering SG)]
        [Space(5)]
        [Toggle(_SSS_ON)] _UseSSS("启用 SG SSS", Float) = 0
        _SSSColor("SSS 颜色 (散射色)", Color) = (1, 0.2, 0.1, 1)
        _SSSScale("SSS 强度 (散射半径)", Range(0.0, 10.0)) = 1.0
        _SSSCurvatureScale("曲率影响系数", Range(0.0, 5.0)) = 1.0
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "zEPH_H_Dev_Subsurface_Surface_Backup_TerminatorFix.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _SSS_ON            // 是否开启 SSS
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "zEPH_H_Dev_Subsurface_Lighting_Backup_TerminatorFix.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        Pass
        {
            ...
            Varyings ShadowPassVertex(Attributes input)
            {
                ...
                // [Fix] 手动实现 GetShadowPositionHClip，因为它不是 URP 标准库的公共函数
                float3 positionWS = TransformObjectToWorld(input.positionOS.xyz);
                float3 normalWS = TransformObjectToWorldNormal(input.normalOS);

                // _LightDirection 由 URP 在渲染 ShadowCaster 时设置
        ...
        Pass
        {
            ...
            {
                SurfaceData surfaceData;
                InitializeSurfaceData(IN.uv, IN.uv, surfaceData);
