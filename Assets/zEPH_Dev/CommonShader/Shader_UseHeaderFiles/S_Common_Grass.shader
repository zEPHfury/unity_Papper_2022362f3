Shader "Custom/Common Grass"
{
    ...
    Properties
    {
        ...
        [Space(15)]
        // ORM: R=Occlusion, G=Smoothness, B=Metallic
        [NoScaleOffset] _ORMAMap("ORM贴图 (R=AO, G=Smoothness, B=Metallic)", 2D) = "white" {}
        _SmoothnessMin("光滑度最小值", Range(0.0, 1.0)) = 0
        _SmoothnessMax("光滑度最大值", Range(0.0, 1.0)) = 1
        _OcclusionStrength("AO 强度", Range(0.0, 1.0)) = 1.0
        _EmissionStrength("自发光强度", Range(0.0, 10.0)) = 0.0
        ...
        [Space(10)]
        [Toggle(_ALPHATEST_ON)] _AlphaClip("开启 Alpha 裁剪", Float) = 1
        [KeywordEnum(B, A)] _AlphaSource("Alpha 来源通道 (ORMA)", Float) = 0 // 选择 Alpha 数据来源：ORM 的 B 通道还是 A 通道
        _Cutoff("裁剪阈值", Range(0.0, 1.0)) = 0.45

        [Space(15)]
        [Header(Advanced)]
        [Enum(UnityEngine.Rendering.CullMode)] _Cull("剔除模式", Float) = 0 // Back
        ...
        [Header(Wind)]
        [Space(5)]
        // [KeywordEnum(Bounds, VertexColor)] _WindMaskSource("Wind Mask Source", Float) = 0
        _WindIntensity("风强度", Float) = 1
        _WindSpeed("风速", Float) = 1.0
        _WindWeightScale("风权重缩放", Float) = 1.0
        _WindWeightPower("风权重幂次", Float) = 1.0
        // _BoundsMin("Bounds Min (OS)", Vector) = (0, 0, 0, 0)
        // _BoundsMax("Bounds Max (OS)", Vector) = (1, 1, 1, 0)
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Common_Grass_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _ALPHASOURCE_A     // Alpha 来源是否为 A 通道
            // #pragma shader_feature_local _WINDMASKSOURCE_BOUNDS _WINDMASKSOURCE_VERTEXCOLOR // 风动遮罩来源
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Common_Grass_Light.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            Varyings LitPassVertex(Attributes IN)
            {
                ...
                // >>> 风动计算 >>>
                float windWeight = 0;
                // #if defined(_WINDMASKSOURCE_VERTEXCOLOR)
                    windWeight = IN.color.r; // 使用顶点颜色 R 通道
                // #else
                //    float3 uvw;
                //    BoundingBoxBasedUVW_float(IN.positionOS.xyz, _BoundsMin, _BoundsMax, uvw);
                //    windWeight = saturate(uvw.y);
                // #endif

                float3 windOffset = SimpleGrassWind(posInputs.positionWS, _WindIntensity, windWeight, _WindSpeed);
                posInputs.positionWS += windOffset;
                // 重新计算裁剪空间位置
        ...
        Pass
        {
            ...
            // #pragma shader_feature_local _WINDMASKSOURCE_BOUNDS _WINDMASKSOURCE_VERTEXCOLOR
            ...
            Varyings ShadowPassVertex(Attributes input)
            {
                ...
                // #else
                //    float3 uvw;
                //    BoundingBoxBasedUVW_float(input.positionOS.xyz, _BoundsMin, _BoundsMax, uvw);
                //    windWeight = saturate(uvw.y);
                // #endif
                
                float3 windOffset = SimpleGrassWind(positionWS, _WindIntensity, windWeight, _WindSpeed);
            ...
            // 阴影 Pass 片元着色器
            half4 ShadowPassFragment(Varyings IN, float facing : VFACE) : SV_Target
            {
                UNITY_SETUP_INSTANCE_ID(IN);
                
                // 获取 Alpha 值并进行裁剪
                #if defined(_ALPHATEST_ON)
                    half alpha = GetAlpha(IN.uv);
                    DoAlphaClip(alpha);
                #endif
