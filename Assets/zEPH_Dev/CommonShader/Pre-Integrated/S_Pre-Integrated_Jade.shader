Shader "Custom/Common Jade"
{
    Properties
    {
        ...
        [KeywordEnum(B, A)] _MetallicChannel("Metallic Channel Select", Float) = 0
        ...
        _JadeShadowStrength("Jade Shadow Strength", Range(0.0, 1.0)) = 0.6
        _JadeShadowSoftness("Jade Shadow Softness", Range(0.0, 1.0)) = 1.0
        
        [Space(10)]
        [Header(Transmission)]
        [Space(5)]
        [KeywordEnum(B, A)] _TransmissionChannel("Transmission Channel Select", Float) = 0
        ...
        _FakeLightCenterBrightness("Fake Light Center Brightness", Float) = 1.0
        _FakeLightEdgeBrightness("Fake Light Edge Brightness", Float) = 0.8
        _FakeLightCenterRangeOffset("Fake Light Center Range Offset", Float) = 2.0

        [Header( Custom Specular)]
        [Space(5)]
        _SpecularEdgeHardness("Specular Edge Hardness", Range(0.0, 1.0)) = 0.9
        _SpecularRange("Specular Range", Range(0.0, 9.0)) = 1.25
        ...
        [Header(Emission)]
        [Space(5)]
        [Toggle(_EMISSION_ON)] _EmissionEnabled("Emission Enabled", Float) = 0.0
        _EmissionSpeed("Emission Speed", Float) = 5.0
        _EmissionIntensity("Emission Intensity", Float) = 1.0
        _EmissionMinBrightness("Emission Min Brightness", Range(0.0, 1.0)) = 0.0
        _EmissionMaxBrightness("Emission Max Brightness", Range(0.0, 1.0)) = 1.0
    }
    ...
    SubShader
    {
        ...
        CBUFFER_START(UnityPerMaterial)
            ...
            half _EmissionMinBrightness;
            half _EmissionMaxBrightness;
        CBUFFER_END
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _TRANSMISSIONCHANNEL_A _TRANSMISSIONCHANNEL_B
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                #if defined(_TRANSMISSIONCHANNEL_A)
                    half thicknessMap = orma.a;
                #else
                    half thicknessMap = orma.b;
                #endif
                half thickness = lerp(_ThicknessRemap.x, _ThicknessRemap.y, thicknessMap);
                ...
                float sinTime = sin(_Time.y * _EmissionSpeed) * 0.5 + 0.5;
                half emissionCycle = lerp((half)_EmissionMinBrightness, (half)_EmissionMaxBrightness, (half)sinTime);
                half3 finalAlbedo = lerp(jadeAlbedoRGB, metallicAlbedoRGB, metallic);
                half3 emission = finalAlbedo * emissionCycle * _EmissionIntensity;
                finalColor += emission;
                #endif
