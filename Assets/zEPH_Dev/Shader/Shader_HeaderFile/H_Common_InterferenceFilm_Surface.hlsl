...
CBUFFER_START(UnityPerMaterial)
    ...
    half _InterferenceFilmIntensity;
    half _Steps;
...
// InterferenceFilm
// ---------------------------------------------------------------------------
half3 InterferenceFilm(half3 PN, half3 CV, half steps)
{
    half3 k = normalize(half3(1.0, 1.0, 1.0));
    half t = dot(PN, CV) * PI * steps;
    half3 C = half3(0.5, 0.5, 1.0);
    half3 res = C * cos(t) + cross(k, C) * sin(t) + k * dot(k, C) * (1.0 - cos(t));
    return res;
}

// ---------------------------------------------------------------------------
// 核心函数：初始化 SurfaceData (基础版本，不含干涉)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 重载函数：初始化 SurfaceData (包含干涉效果)
// ---------------------------------------------------------------------------
void InitializeSurfaceData(float2 uv, half3 normalWS, half3 viewDirectionWS, out SurfaceData outSurfaceData)
{
    // 1. 调用基础初始化
    InitializeSurfaceData(uv, outSurfaceData);
    
    // 2. 叠加 Interference Film 效果
    half3 interference = InterferenceFilm(normalWS, viewDirectionWS, _Steps);
    // 使用 lerp 控制叠加强度：lerp(原色, 原色+干涉, 强度)
    outSurfaceData.albedo = lerp(outSurfaceData.albedo, outSurfaceData.albedo + interference, _InterferenceFilmIntensity);
}
