...
CBUFFER_START(UnityPerMaterial)
    ...
    half _MetallicMapMin;
    half _MetallicMapMax;
    half _SmoothnessMin;
    half _SmoothnessMax;
...
void InitializeSurfaceData(float2 uv, float2 emissionUV, out SurfaceData outSurfaceData)
{
    ...
    // 4. 物理属性
    outSurfaceData.metallic = saturate(lerp(_MetallicMapMin, _MetallicMapMax, ormSample.b));
...
//     half3 emissionSample = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, emissionUV).rgb;
//     outSurfaceData.emission = emissionSample * _EmissionColor.rgb * _EmissionScale;
// #else
