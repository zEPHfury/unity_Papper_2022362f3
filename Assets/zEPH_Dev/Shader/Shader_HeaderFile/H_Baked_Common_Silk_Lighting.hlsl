...
// ---------------------------------------------------------------------------
half3 LightingSilk(Light light, InputData inputData, SurfaceData surfaceData, float3 tangentWS, float3 bitangentWS)
{
    ...
    half3 diffuse = surfaceData.albedo * lightColor * NoL;
    ...
    half3 specular = _AnisoColor.rgb * aniso * lightColor * _AnisoStrength;
...
half4 SilkFragmentPBR(InputData inputData, SurfaceData surfaceData, float3 tangentWS, float3 bitangentWS)
{
    ...
    half3 color = inputData.bakedGI * surfaceData.albedo; // Ambient / GI
    color += LightingSilk(mainLight, inputData, surfaceData, tangentWS, bitangentWS);
    ...
    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
    {
        ...
        color += LightingSilk(light, inputData, surfaceData, tangentWS, bitangentWS);
    }
#endif
    ...
    // 4. Baked Specular Compensation (Fake Specular)
        ...
        // 叠加高光颜色
        color += _AnisoColor.rgb * aniso * fakeLightColor * _AnisoStrength;
    #endif
