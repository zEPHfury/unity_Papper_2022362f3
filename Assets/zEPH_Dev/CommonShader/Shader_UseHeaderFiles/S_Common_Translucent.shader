Shader "Custom/Common Translucent"
{
    // ---------------------------------------------------------------------------
    // 属性定义
    // ---------------------------------------------------------------------------
    Properties
    {
        ...
        [Space(15)]
        [Header(TranslucentRefraction)]
        [Space(5)]
        _Opacity("不透明度", Range(0.0, 1.0)) = 0.9
        [Toggle(_REFRACTION)] _UseRefraction("启用折射 (需开启 URP Opaque Texture)", Float) = 0
        _RefractionStrength("折射强度", Range(0.0, 1.0)) = 0.1
        ...
        // [Toggle] _ZWrite("开启深度写入 (折射建议开启)", Float) = 0
    }
    ...
    SubShader
    {
        ...
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        #include "H_Common_Translucent_Surface.hlsl"
        // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ENDHLSL
        ...
        Pass
        {
            ...
            Blend SrcAlpha OneMinusSrcAlpha // 混合模式：标准半透明混合
            // ZWrite [_ZWrite] // 由材质属性控制
            ZWrite On // 强制开启深度写入，以解决折射时的乱序问题
            ...
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            #include "H_Common_Translucent_Lighting.hlsl"
            // User-Define HLSL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
